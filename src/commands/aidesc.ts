...
export default class Aidesc extends Command {
  static override description = "Generate AI-powered commit descriptions based on current changes";
  ...
  public async run(): Promise<void> {
    ...
    const diff = await $`jj diff --git ${selectedChanges.map((change) => change.path)}`.text();
