  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="yellow" padding={1}>
      ...
      <Box marginTop={1} flexDirection="row" gap={2}>
        ...
        <Text color={selected === "no" ? "red" : "gray"}>{selected === "no" ? ">" : " "} [N]o</Text>
      </Box>
      <Box marginTop={1}>
...
const MergeRevisionDisplay = ({ revToMerge }: { revToMerge: RevisionToMerge }) => {
  ...
  return (
    <Box flexDirection="column">
      {revisionsToMerge.map((revToMerge) => (
        <MergeRevisionDisplay key={revToMerge.rev.changeId} revToMerge={revToMerge} />
      ))}
      <Text>~</Text>
    </Box>
  );
