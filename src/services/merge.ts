...
export const [emitMergeEvent, useMergeEvent] =
	createEventHook<MergeEventTypes>();

// Log emitted merge events
const logEvents = <K extends keyof MergeEventTypes>(
	type: K,
	data: MergeEventTypes[K],
) => {
...
export class RevisionToMerge {
	...
	async waitForMergeability() {
		...
		return pWaitFor(
			async () => {
				...
				log.debug(
					`Revision still not mergeable: ${this.rev.changeId}, updating...`,
				);
	...
	async canBeMerged() {
		...
		if (!this.rev.bookmark) {
			log.debug(
				`Revision has no bookmark, cannot be merged: ${this.rev.changeId}`,
			);
			return false;
		}
		...
		if (
			![PullRequestReviewDecision.Approved, null, undefined].includes(
				this.mergeData.reviewDecision,
			)
		) {
			log.debug(
	...
	async update(emitEvent = true) {
		if (!this.isEligible()) {
			log.debug(
				`Skipping update for ineligible revision: ${this.rev.changeId}`,
			);
			return;
		}
...
export class StackMerge {
	...
	async execute() {
		...
		const outdatedRevisions = this.mergeRevisions.filter(
			(rev) => rev.rev.remoteOutdated,
		);
